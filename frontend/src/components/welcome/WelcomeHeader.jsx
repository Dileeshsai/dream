...
const WelcomeHeader = () => {
  ...
  return (
    <header
      className="w-full px-4 py-2 border-b border-gray-100"
      style={{
      ...
      <div className="w-full flex flex-row items-center justify-between gap-2 md:gap-4 relative z-30">
        ...
        <div className="flex flex-row items-center gap-4 min-w-fit justify-center md:justify-start">
          ...
          <div className="flex flex-col leading-tight ml-1">
            <span className="font-extrabold text-2xl md:text-3xl tracking-tight" style={{color:'#2563eb',fontFamily:logoFont,letterSpacing:'-0.5px',lineHeight:'1.1',textTransform:'uppercase'}}>UNITY</span>
            <span className="font-bold text-lg md:text-xl tracking-wide" style={{color:'#06b6d4',fontFamily:logoFont,marginTop:-2,letterSpacing:'0.5px',textTransform:'uppercase'}}>Nest</span>
          </div>
        </div>
        {/* Desktop Nav & Auth */}
        {!isMobile && (
          <>
                ...
                {navLinks.map((link) => (
                  <li key={link.to}>
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        `px-4 py-2 rounded-full nav-creative-link transition-all duration-200 ${
                          isActive ? 'active bg-blue-100 font-bold scale-105' : 'text-gray-700 hover:bg-cyan-50'
                        }`
                      }
                      style={{ fontFamily: navFont, fontWeight: 700, letterSpacing: '0.04em', background: 'transparent', fontSize: '1.08rem' }}
                    >
                      {link.label}
                    </NavLink>
                  </li>
            ...
            <div className="flex gap-3 min-w-fit justify-center md:justify-end">
              <Link to="/login">
                <Button
                  className="bg-white border border-blue-200 text-blue-700 font-semibold px-6 py-2 rounded-full shadow-sm hover:bg-blue-50 hover:text-blue-800  transition-all text-base"
              ...
              <Link to="/register">
                <Button
                  className="bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-bold px-6 py-2 rounded-full  hover:from-blue-700 hover:to-cyan-600 hover:scale-105 transition-all text-base border-0"
                  style={{ fontFamily: headerFont, letterSpacing: '0.02em' }}
                >
                  Sign up
                </Button>
              ...
              <SheetContent side="left" className="p-0 w-64 transition-all duration-300 ease-in-out">
                <div className="flex flex-col gap-6 p-6">
                  ...
                  <nav className="flex flex-col gap-4">
                    {navLinks.map((link) => (
                      <NavLink
                        ...
                        className={({ isActive }) =>
                          `px-2 py-2 rounded transition-colors duration-200 text-lg ${
                            isActive
                  ...
                  <div className="flex flex-col gap-2 mt-6">
                    <Link to="/login"><button className="border border-gray-400 rounded px-4 py-2 text-gray-800 bg-white hover:bg-gray-100 transition text-base">Log in</button></Link>
                    <Link to="/register"><button className="bg-blue-600 text-white rounded px-5 py-2 font-semibold hover:bg-blue-700 transition text-base">Join Us</button></Link>
                  </div>
                </div>
              </SheetContent>
            </Sheet>
